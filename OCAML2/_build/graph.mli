...
(* Add an edge given the ids of origin (id1) and destination (id2).
 * If an edge already exists between id1 and id2, its label is replaced by the ancient one + the new one. *)
val add_edge_with_addition: ('v, int) graph -> id -> id -> int -> unit
...
val map_hashtbl : ('a, 'b) Hashtbl.t -> ('b -> 'c) -> ('a, 'c) Hashtbl.t
